...
sh pickcols.awk <(python pythonScripts/select-admixture.py "CEU" "YRI" | awk '{print $2; end}' | sed '$d' - ) <( tail -n +6 ${Data}/hapmap3_r2_b37_fwd_phased.trio-parents.recode.vcf) | tr ' ' \\t > admix-YRI\|CEU-input.vcf 
...
sh pickcols.awk <(python pythonScripts/prune-admixture.py admix-YRI\|CEU-output.ids) <(cat admix-YRI\|CEU-output.vcf) | tr ' ' \\t > admix-YRI\|CEU-output_pruned.vcf
